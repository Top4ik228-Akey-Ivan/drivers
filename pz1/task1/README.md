...
Необходимо считать данные в размере 100 байт из утсройства. Отследить системные вызовы с помощью strace
...
brk(0x2ba86d00)
```
mprotect — установка прав доступа к страницам памяти
```
mprotect(0x4ad000, 20480, PROT_READ)
```
## Файловая подсистема
...
read — чтение 100 байт нулевых данных
```
read(3, "\0\0\0\0...", 100) = 100
```
write — вывод строки в стандартный поток вывода
```
